...
def run_data_integrity_report(df_path):
    try:
        ...
        cat_features = ["administrative_status", "marketing_status", "approved_for_hospital_use", 
                      "reimbursement_rate", "marketing_authorization_status", 
                      "marketing_authorization_process", "group_related",
                      "main_route_of_administration", "pareto_companies"]
        
        dataset= Dataset(df, label= 'price', cat_features = cat_features)
        ...
        output_folder = "training_reports"
...
def run_validation_train_test_split_report(X_train, y_train, X_test, y_test):
    try:
        ...
        # Create Deepchecks Datasets
        train = Dataset(X_train, label=y_train,cat_features = cat_features)
        test = Dataset(X_test, label=y_test, cat_features = cat_features)
